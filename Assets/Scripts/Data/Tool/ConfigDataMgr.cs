...
namespace act.data
{
    ...
    public static class ConfigDataMgr
    {
        ...
        public static List<T> ReturnObjectArraryBySplitString<T>(string sentence, char splitSymbol)
        {
            List<T> Tlist = new List<T>();
            string[] phrases = sentence.Split(splitSymbol);
            foreach (string item in phrases)
            {
                ...
                {
                    typeT = (T)System.Convert.ChangeType(item, typeof(T));
                }
                Tlist.Add(typeT);
            }
            return Tlist;
