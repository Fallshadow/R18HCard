...
namespace data
{
    ...
    public class ConfigRow
    {
        ...
        public bool equalsValue(int col, int value)
        {
            if (col == -1)
                return false;

            int temp = 0;
            if (!System.Int32.TryParse(_data[col], out temp))
                return false;
            return (value == temp);
        }
        ...
        public bool getUintValue(System.Enum key, out uint value)
        {
            int i;
            bool b = getIntValue(key, out i);
            value = (uint)i;
            return b;
        }
